...
export default function Home() {
  ...
  const pacientesRecentes = pacientes?.length ? [...pacientes].reverse().slice(0, 3) : [];
              ...
              <tbody>
                {pacientesRecentes.length === 0 ? (
                  ...
                  pacientesRecentes.map((p, idx) => (
                    <tr key={p._id} style={{ borderBottom: idx === pacientesRecentes.length - 1 ? 'none' : '1px solid hsl(var(--border))' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {proximasSessoes.length === 0 ? (
              ...
              proximasSessoes.map((a, idx) => (
                <div key={a._id} className="card" style={{ padding: '1rem' }}>
